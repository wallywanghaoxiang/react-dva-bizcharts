...
export default class QuestionShowCard extends PureComponent {
  ...
  mainQuestionItem(newData, mainData, jsx,id) {
    ...
    let startIndex=this.getType()=="TWO_LEVEL"?0:subStartIndex;//如果是二层题型，主题干的ID和第一小题一致，防止左侧导航栏定位错误
        ...
        if (newData[i].components.name == 'stemVideo') {
          ...
          if (mainData.mainQuestion[newData[i].components.name]) {
            if(newData[i].components.textButton === "stemVideoText" && mainData.mainQuestion[newData[i].components.textButton]!= "" && mainData.mainQuestion[newData[i].components.textButton]!= null){
              jsx.push(<div id={String(mainIndex)+"-"+ String(questionIndex)+"-"+ String(this.getSubNum(startIndex))+"-"+this.getType()}>
                <StemVideoText
                  text={mainData.mainQuestion[newData[i].components.textButton]}
          ...
          //主题干图片
          jsx.push(<div id={String(mainIndex)+"-"+ String(questionIndex)+"-"+ String(this.getSubNum(startIndex))+"-"+this.getType()}>
                 {/* <a>{'6link' + String(mainIndex)+"-"+ String(questionIndex)+"-"+ String(this.getSubNum(startIndex))+"-"+mainIndex+"+"+questionIndex+"+"+subStartIndex+this.getType()}</a>  */}
                <StemImage
                id={mainData.mainQuestion[newData[i].components.name]}
  ...
  renderFooter(questionIndexData, sIndex) {
    ...
    if (!role){
      answer = <AnalysisModal dataSource={questionIndexData} masterData={subAnswerIndex} mark={this.props.subFallMark*this.numberSub} report={true} online={true} hiddenRecordAnswer={"online"}  />
    }else{
      answer =<AnswersModal
