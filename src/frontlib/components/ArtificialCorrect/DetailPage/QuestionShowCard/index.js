...
export default class QuestionShowCard extends PureComponent {
  ...
  saveManual=(id,manualDetail,manualScore)=>{
    const {dispatch,checkedID,checkClassId,subQuestions} = this.props;
    dispatch({
...
console.log(manualScore)
    return (
      <div className={styles.footerDetail}>
        <div className="scoreDetail"><div className="scoreBorders">{formatMessage({id:"app.text.intelligence.score",defaultMessage:"智能评分"})}：<span>{score}分</span><b>|</b>{formatMessage({id:"app.text.Deviation.correction.score",defaultMessage:"纠偏得分"})}：<span>{manualScore!==null?manualScore||0:'--'}</span></div></div>
    ...
    return (
      <div>
        <div className={styles.card_detail} key="logo">
          ...
          {questionIndexData.patternType == 'NORMAL'?<div className="nobacks blueBg"  onMouseOver={id => {
                if(id!==this.props.id) {
