...
export default class DetailPage extends PureComponent {
  ...
  render() {
    const { paperData, showData, classNum,subQuestions,checkedID,checkClassId } = this.props;
    const { id } = this.state;
    let sub = [];
    let type = '';
      ...
      if (!Item.type || (Item.type != 'RECALL' && Item.type != 'SPLITTER')) {
        ...
        if ('COMPLEX' != type) {
          Item.questions.map((Item1, index1) => {
            let questionID = subQuestions.filter(vo=>vo.subquestionNo===Item1.id)
            if(type==='TWO_LEVEL') {
              Item1.data.subQuestion.map((vo2=>{
                 questionID = subQuestions.filter(vo=>vo.subquestionNo===vo2.id)
              }))
            }
            console.log(questionID,Item1.id)
            if (Item1&&questionID.length>0) {
              sub.push(
                <div key={index+"card_"+index1} className={classNames(styles.card, "CorrectCard")}>
                  <QuestionShowCard
                    dataSource={Item1}
          ...
          for (let i in Item.pattern.groups) {
            let questionID = subQuestions.filter(vo=>vo.subquestionNo===Item.questions[0].data.groups[i].id)
            if(Item.questions[0].data.groups[i].data.patternType==='TWO_LEVEL') {
              Item.questions[0].data.groups[i].data.subQuestion.map((vo2=>{
                questionID = subQuestions.filter(vo=>vo.subquestionNo===vo2.id)
             }))
            }
            console.log(questionID,Item.questions[0].data.groups[i].id)
            if (Item.questions[0]&&questionID.length>0) {
              sub.push(
    ...
    console.log(showData);
    return (
      <div className="DetailPage ArtificialPage">
        {sub}
        <div className="end">
