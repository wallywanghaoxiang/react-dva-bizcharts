...
export default class DetailPage extends PureComponent {
  ...
  render() {
    const { paperData, showData, classNum,V_ANSWER_ANALYSIS } = this.props;
    const { id } = this.state;
    let sub = [];
    let type = '';

    //! TODO VB-8574 学生报告页答案解析版本区分控制
    // 1. 当前校区有 V_ANSWER_ANALYSIS 权限，全部可见
    // 2. 无权限时，仅前两个可见
    // 答案解析计数
    let analysisModalIndex = 0;

    paperData.paperInstance.forEach((Item, index) => {
              ...
              sub.push(
                <div key={`${index}card_${index1}`} className="card">
                  <QuestionShowCard
                    ...
                    analysisPromission={V_ANSWER_ANALYSIS || analysisModalIndex < 2}
                  />
                </div>
              );
