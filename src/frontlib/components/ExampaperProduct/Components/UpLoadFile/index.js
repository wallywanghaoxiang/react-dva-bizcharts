...
class UploadFile extends Component {
  ...
  componentDidMount() {
    ...
    let self = this;
    emitter.addListener('stopotherUpLoadFile', x => {
      console.log(x)
      if(x!= self.state.audioUrl){
        self.pauseSound();
      }else{
        if(self.audioValue){
          self.thenStart();
  ...
  start() {
    ...
    console.log(self.audioValue.duration)
  ...
  playSound() {
    emitter.emit("stopotherUpLoadFile",this.state.audioUrl);
  ...
  render() {
    ...
    const { style, backgroundStyle, contentStyle} = this.props;
    return (
      <div style={style}>
            <audio
            ...
            />
        {
          audioUrl ? <div className="uploadFiles" style={contentStyle}>
                            <div className="uploadSingle" style={backgroundStyle}>

                              <IconButton iconName="icon-wave" type="" className="textNone" style={{display: "inline-block"}}/>
                              {name}<span className="waveInfo">{countDown(Math.round(duration))}</span>
                          ...
                          :
                          <CustomUpload onSuccess={this.handleSuccess} accept='audio/mp3'/>
        }
