 ...
 * @Author    tina
 * @DateTime  2018-11-20
 * @copyright 7.2.9 题后指导图片控件（guideSuffixImageControl）
 */
...
class GuideSuffixImageControl extends Component {
  constructor(props) {
    super(props);
    this.state = {
      visible: true,
  ...
  componentDidMount() {
    ...
    if (editData) {
      ...
      if (editData && editData.patternType != "COMPLEX") {
        fileID = editData.mainQuestion.guideSuffixImage;

      } else if (editData && editData.patternType == "COMPLEX" && editData.groups[mainIndex].data) {
        fileID = editData.groups[mainIndex].data.mainQuestion.guideSuffixImage;
      }
      if (fileID) {
        fetchPaperFileUrl({
          fileId: fileID
    ...
    return (
      <div className="demon">     
         <FormItem label={data.params.label}>
                {getFieldDecorator('GuideSuffixImage'+subIndex+index2, {
                  initialValue: id,
...
export default GuideSuffixImageControl;
