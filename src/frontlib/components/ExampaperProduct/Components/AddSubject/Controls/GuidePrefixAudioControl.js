...
class GuidePrefixAudioControl extends Component {
  constructor(props) {
    super(props);
    this.state = {
      visible: true,   
  ...
  componentDidMount() {
     ...
     if(editData) {
      const mainIndex=index2=='all'?'1':index2        
        if(editData&&editData.patternType!="COMPLEX"&&editData.mainQuestion.guidePrefixAudio) {   
            fileID=editData.mainQuestion.guidePrefixAudio;
            stemAudioTime=editData.mainQuestion.guidePrefixAudioTime
           ...
           fileID=editData.groups[mainIndex].data.mainQuestion.guidePrefixAudio;
           stemAudioTime=editData.groups[mainIndex].data.mainQuestion.guidePrefixAudioTime
    ...
    return (
      <div className="demon">
        <FormItem label={data.params.label}>
          {getFieldDecorator('GuidePrefixAudioControl'+index2+subIndex, {
            initialValue: id,
...
export default GuidePrefixAudioControl;
