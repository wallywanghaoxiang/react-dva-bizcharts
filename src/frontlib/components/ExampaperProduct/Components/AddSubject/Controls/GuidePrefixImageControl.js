...
class GuidePrefixImageControl extends Component {
  constructor(props) {
    super(props);
    this.state = {
      visible: true,
  ...
  componentDidMount() {
    ...
    if (editData) {
      ...
      if (editData && editData.patternType != "COMPLEX") {
        fileID = editData.mainQuestion.guidePrefixImage;

      } else if (editData && editData.patternType == "COMPLEX" && editData.groups[mainIndex].data) {
        fileID = editData.groups[mainIndex].data.mainQuestion.guidePrefixImage;
      }
      if (fileID) {
        fetchPaperFileUrl({
          fileId: fileID
    ...
    return (
      <div className="demon">     
         <FormItem label={data.params.label}>
                {getFieldDecorator('guidePrefixImage'+subIndex+index2, {
                  initialValue: id,
...
export default GuidePrefixImageControl;
