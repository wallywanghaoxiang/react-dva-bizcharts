...
class H5Recorder {
  ...
  constructor(options) {
    ...
    this.duration = 5000 / 200; // 初始录音时长5秒,现在定时器每200毫秒检查一次
    this.hint = true; // 是否播放录音提示音的标志
    this.resourceType = 'off'; // 是否播放录音提示音的标志
    this.nsx = null; // 降噪开关
    this.list = options.recorderAudioList; // 存放临时录音文件
    this.socket = new socket_VB(this);
    this.count = 0; // 计时器，秒
    this.isStop = false; // 是否已经点过stop
    ...
    this.callbackDeviceSateChanged = null; // 麦克风掉落事件

    this.tokenId = null;
    this.core_timeout = 20000; // 评分超时时间默认20秒
  ...
  startMicphone = ()=>{
    if( navigator.mediaDevices && navigator.mediaDevices.getUserMedia ){
      navigator.mediaDevices.getUserMedia({audio:true,video:false}).then(stream=>{
        ...
        this.enumerateDevices("bind");
      }).catch(()=>{
        if (this.Callback_onError) {
          this.Callback_onError({ errId: 10011, error: 'no audio device!' });
        } else {
   ...
   * 触发状态变更的条件
   * type : 触发的方式
   *    auto   : 自动触发，一般是原生的api触发的
   *    manual : 手动触发的，如点击重试按钮、或者获取录音功能后主动调用
   */
  enumerateDevices = (type)=>{
    console.log("触发设备变更的方法===",type);
    // 此方法短时间内只能执行一次
    if( typeof(this.callbackDeviceSateChanged) === "function" ){
      this.callbackDeviceSateChanged(type);
    }
  }
  ...
  onDeviceStateChanged = callback => {
    if (typeof callback === 'function') {
      this.callbackDeviceSateChanged = callback;
    }
  };
  ...
  /**
   *取消按钮，在录音中，或等待评分时，取消当前评分的状态，回到下次开始录音状态
   *
   * @author tina.zhang
   * @date 2019-01-16
   * @memberof H5Recorder
  ...
  cancel() {
    ...
    this.recording = false;
    if (this._timer_wait_res) {
      clearTimeout(this._timer_wait_res);
      this._timer_wait_res = null;
    }
    if (this._timer_stop) {
   ...
   * params参数：
   * duration: Int, //录音时长（秒）
   * hint: Bool, //是否播放提示音 已废弃
   * resourceType,//string 播放提示音
  ...
  start(params) {
    ...
    if (!this.record_worker && this.Callback_onError) {
      this.Callback_onError({ errId: 10011, error: 'no record_worker!' });
      return;
    }
    if (params.data) {
      params = params.data;
    ...
    if (params.request.hasOwnProperty('attachAudio') == false) {
      params.request.attachAudio = true;
    }
    temp.request = params.request;
    this.duration = (params.duration * 1000) / 200;
  ...
  stop = async () => {
    if (!this.recording || this.isStop) return;
    if (this.count < 10) {
      // 录音小于两秒时，等到1秒才停止
      this.isStop = true;
      await this.delay(1000 - 100 * this.count);
    }
    let x = this;
    this.isStop = false;
    this.recording = false;
    ...
    console.log('record start core timer');
    this._timer_wait_res = setTimeout(() => {
      console.error('core timeout, and no onScoreError callback.');
      if (x.Callback_onError) {
        x.Callback_onError({ errId: 10010, error: 'core timeout, and no onScoreError callback.' });
      }
    }, 20000);
    ...
    if (this.Callback_onStop) {
      this.Callback_onStop({ tokenId: this.tokenId, timer: this._timer_wait_res });
    }
  };
  ...
  onEval(callback) {
    let x = this;
    if (typeof callback === 'function') {
      this.socket.Callback_onEval = data => {
        if (data.indexOf('errId') > 0) {
          if (x.Callback_onError) x.Callback_onError(data);
        } else {
          callback(data);
        }
      };
  ...
  onError(callback) {
    if (typeof callback === 'function') {
      ...
      this.socket.Callback_onError = a => {
        this.cancel();
  ...
  _gotStream(stream) {
    ...
    let blob_record_worker = new Blob([record_worker_s], { type: 'application/javascript;' });
    this.record_worker = new Worker(window.URL.createObjectURL(blob_record_worker));
    // this.record_worker = new Worker("/recorderWorker.js");
    this.record_worker.onmessage = e => {
      if (e.data.command == 'blob') {
        ...
        // 		localStorage["myaudio"+x.tokenId.toString()]=event.currentTarget.result;
        //     };
        ...
        // 非压缩
        this.socket.send_audio(0, result);
  ...
  _startRecord(request) {
    ...
    x.recording = true;
    x.count = 0;
    x._timer_stop = setInterval(() => {
      x.count += 1;
      if (x.count >= ~~x.duration) {
        x.stop();
      }
    }, 200);
    ...
    url = Common.getAudioSrc(x.resourceType);
    if (url) {
      x._playding(url); // 播放叮声
    }
...
function record(inputBuffer) {
    ...
    for (var i = 0; i < l; i++) {
        ...
        //频率为10Hz返回录音音量
        if (_count > 1600) {
            var v = Math.max(Math.abs(_maxValue), Math.abs(_minValue));
            this.postMessage({
                'command': 'volume',
                'volume': v,
                'max': _maxValue,
