...
const messages = defineMessages({
  ...
  naturalClassTit: { id: 'app.campus.manage.basic.natural.class.title', defaultMessage: '行政班' },
  swapTip2: { id: 'app.campus.manage.class.config.swap.tip2', defaultMessage: '学生异动期' },
  swapTip3: {
    id: 'app.campus.manage.class.config.swap.tip3',
...
@connect(({ campusmanage, loading }) => {
  ...
  const teacherLoading = loading.effects['campusmanage/teacherList'];
...
class ClassConfigure extends Component {
    ...
    dispatch({
      ...
      callback: res => {
        if (res.responseCode === '200') {
          ...
          const gradeId = dataJson.length > 0 ? dataJson[0].id : '';
          const gradeCode = dataJson.length > 0 ? dataJson[0].grade : '';
          const gradeValue = dataJson.length > 0 ? dataJson[0].gradeValue : '';
          this.setState(
            {
  ...
  getNaturalClass = () => {
    const { dispatch } = this.props;
    const { currentGradeId, isOpen } = this.state;
    const params = {
      gradeId: currentGradeId,
    };
    dispatch({
      type: 'campusmanage/naturalClass',
      payload: params,
      callback: data => {
          ...
          data.forEach(it => {
            showData.forEach(obj => {
              if (it.classIndex === obj.classIndex) {
                ...
                obj.naturalClassId = it.naturalClassId;
  ...
  bingManager = (teacherId, item) => {
    const { dispatch } = this.props;
    const { currentGradeId, currentGradeCode } = this.state;
    const campusId = localStorage.getItem('campusId');
    ...
    const params = {
      ...
      teacherId, // 教师ID
      naturalClassId: item.naturalClassId, // 行政班ID
    };
    dispatch({
      type: 'campusmanage/bingNaturalClassManager',
      payload: params,
      callback: res => {
        if (res.responseCode === '200') {
  ...
  dissolutionClass = item => {
    ...
    Modal.confirm({
      ...
      onOk: () => {
        ...
        dispatch({
          type: 'campusmanage/dissolutionNaClass',
          payload: params,
          callback: res => {
            ...
            if (resJson.responseCode === '200') {
              ...
              this.getNaturalClass();
              this.getTeacherList();
            } else if (resJson.responseCode === '460') {
              // 有学生数据
              Modal.confirm({
                content: (
                  <div className="cont">
  ...
  render() {
    const showTeacherClassNavigation = localStorage.getItem('showTeacherClassNavigation');
    ...
    const content = (
      <div>
          ...
          <div className={styles.showSwap}>
            ...
            <span className={styles.tips2}>
              {Number(lastDays) === 1
                ...
                : `${lastDays}${formatMessage(messages.day)}`}
            </span>
          </div>
        )}
        ...
        <div className={styles.setMoveTit}>
          ...
          <span className={styles.naturalClass}>{formatMessage(messages.naturalClassTit)}</span>
        ...
        <div>
          ...
          >
            <Radio.Button value={1}>1{formatMessage(messages.day)}</Radio.Button>
            <Radio.Button value={3}>3{formatMessage(messages.day)}</Radio.Button>
            <Radio.Button value={7}>7{formatMessage(messages.day)}</Radio.Button>
          </Radio.Group>
        </div>
      ...
      <div className={styles.classConfig}>
        <div className={styles.top}>
          <div className={styles.tit}>
            ...
            <span className={styles.subTit}>{formatMessage(messages.naturalClassTit)}</span>
          </div>
        ...
        {!initLoading && showNoviceNavigation && isFirstGrade && !naturalClassList.length > 0 && (
          <NoviceNavigation
            swap={lastDays && Number(lastDays) > 0}
            onCloseNoviceNavigation={() => {
              const classNavi = `${teacherId}0`;
              localStorage.setItem('showTeacherClassNavigation', classNavi);
              this.setState({
