...
@connect(({ papermanage, release, login }) => {
...
class PaperDetail extends Component {
  ...
  render() {
    ...
    const { showPaper, pageIndex } = this.state;
    const { records, total, size } = paperInfoDetail;
    const grade = gradeList.filter(item => item && item.grade !== '');
    const current = campusList.find(vo => vo.campusId === localStorage.getItem('campusId'));
    console.log(current);
    return (
      <div className={styles.PaperDetail}>
        <div className={styles.paperTitle}>
          ...
          {grade.length > 1 &&
            paperInfoDetail &&
            paperInfoDetail.records &&
            paperInfoDetail.records.length > 0 && (
              <Select
                ...
                {gradeList.map(item => {
                  return (
                    <Option value={item.grade} key={item.grade}>
                      {item.gradeValue || item.paperScopeValue}
                    </Option>
                  );
                })}
              </Select>
            ...
            <List.Item className={item.status === 'N' ? styles.bindStatus : ''}>
              ...
              <div className={styles.operBtn}>
                <Button
                  ...
                  onClick={() => this.previewPaper(item.paperId)}
                  disabled={
                    (current.isAdmin === false &&
                      current.gradeList &&
                      current.gradeList.length === 0) ||
                    (current.isAdmin === false &&
                      current.gradeList &&
                      current.gradeList.length > 0 &&
                      current.gradeList.filter(
                        vo => vo.grade === item.grade || vo.paperScope === item.paperScope
                      ).length === 0)
                  }
                >
