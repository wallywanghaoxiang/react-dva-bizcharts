...
// 对数据进行预处理
@connect(({ permission }) => {
  const { standardList, professionalList } = permission;
  return {
    standardList,
    professionalList,
  };
...
class Permission extends Component {
  ...
  componentDidMount() {
    ...
    // 判断 当期的 type 是否在list 中
    const obj = list.find(item => item.code === type);
    this.setState({
      select: obj ? obj.code : 'V_SINGLE_CLASS_EXAM',
    });
  }
...
// 弹出框方式
Permission.open = type => {
  open(<Permission type={type} />);
};
