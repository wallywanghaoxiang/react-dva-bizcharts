...
import excelStudentPic from '@/assets/teacher/excel_student_pic.png';
...
class ImportTeachersComponent extends Component {
  ...
  downTempMethod = () => {
    ...
    const gender = formatMessage(messages.gender);
    const borrowing = formatMessage(messages.borrowing);
    const params = {
      title: [stuId, name, gender, borrowing],
      key: ['studentClassCode', 'name', 'gender', 'isTransient'],
      data: tableData,
      autoWidth: true,
  ...
  validateData() {
    ...
    const newObject = new_arr.reduce(
      (total, currentValue, currentIndex, arr) => {
        ...
        if (!tag.studentID) {
          ...
          tag.error = formatMessage(messages.genderTip);
        } else if (
          tag.gender !== '男' &&
          tag.gender !== '女' &&
          tag.gender !== 'male' &&
          ...
          tag.error = formatMessage(messages.genderTip1);
    ...
    // 性别
    // const genderObj = newObject['gender'];
    // Object.keys(genderObj).forEach(it => {
    //   const value = genderObj[it];
    //   if (value.length > 1) {
    //     value.forEach(tag => {
    ...
    // 是否走读
    // const borrowingObj = newObject['borrowing'];
    // Object.keys(borrowingObj).forEach(it => {
    //   const value = borrowingObj[it];
    //   if (value.length > 1) {
    //     value.forEach(tag => (tag.warn = formatMessage(messages.studentNameTip1)));
    //   }
    // });
  ...
  initStudentOBJ = item => {
    ...
    const obj = {
      ...
      name: item.name, // 姓名
      gender: item.gender === '男' || item.gender === 'male' ? 'MALE' : 'FEMALE', // 性别
  ...
  handleInput = (rowIndex, cellIndex, value) => {
    ...
    if (rowIndex === 0) {
      ...
      const emptyObj = originArr.find(
        it => it.studentID === '' || it.name === '' || it.gender === '' || it.borrowing === ''
      );
      if (emptyObj) {
        allFillIn = false;
      }
