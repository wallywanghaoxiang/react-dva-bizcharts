...
@connect(({ papermanage }) =>{
    ...
    return { paperInfoList,countPaperPackages,curentPaperPackage,filterWord};
  })
  ...
  setCurrentPaper=(item)=>{
    ...
    dispatch({
      ...
      switchTime=(time)=>{
        ...
        if(currentTime===this.getDay(0)||currentTime===this.getDay(-1)||currentTime===this.getDay(-2))
        {
    ...
    render() {
        const {paperInfoList,countPaperPackages,addTeach,onSearchKey} = this.props;
        ...
        return (
          <div>
            <div className={styles.teacherAll}>
              <span className={styles.itemNumberFirst}><span>{formatMessage(messages.examinationpapers)}</span><i>{totalPaperCount||0}</i><b>{formatMessage(messages.paperset)}</b></span>
              <span className={styles.itemNumber}><span>{formatMessage(messages.Itemnumber)}</span><i>{totalQuestionCount||0}</i><b>{formatMessage(messages.paperItem)}</b></span>
              <span className={styles.itemNumberLast}><span>{formatMessage(messages.Cumulativeuse)}</span> <i>{totalUserCount||0}</i><b>{formatMessage(messages.paperSeconds)}</b></span>                
            ...
            <div className={styles.searchs}><SearchBar placeholder={formatMessage(messages.Search)} onSearch={data=>onSearchKey(data)} maxLength={30} /></div>
            <List
              ...
              renderItem={item => (                  
                <List.Item onClick={()=>this.setCurrentPaper(item)}>
                  ...
                  <div className={styles.paperDetail}>
                    <span className={styles.itemDetail}><span>{formatMessage(messages.examinationpapers)}</span><i>{item.paperCount||0}</i></span>
                    <span className={styles.itemDetail}><span>{formatMessage(messages.Itemnumber)}</span><i>{item.questionCount||0}</i></span>
                    <span className={styles.itemDetail}><span>{formatMessage(messages.Cumulativeuse)}</span> <i>{item.usedCount||0}</i></span>  
                  </div>
