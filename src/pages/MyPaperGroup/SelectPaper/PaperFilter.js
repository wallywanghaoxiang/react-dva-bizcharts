...
@connect(({ papergroup }) => ({
  ...
  years: papergroup.years,
  difficulty: papergroup.difficulty,
  templates: papergroup.templates,
  gradeIndex: papergroup.gradeIndex,
  gradeValue: papergroup.gradeValue,
...
class PaperFilter extends PureComponent {
  ...
  paperFilterData = () => {
    ...
    const fetchPaperTemplates = new Promise((resolve, reject) => {
      dispatch({
        type: 'papergroup/fetchPaperTemplates',
    ...
    const fetchDifficult = new Promise((resolve, reject) => {
      dispatch({
        type: 'papergroup/fetchDifficult',
      })
        .then(() => {
          dataSource2 = [
            {
      ...
      .then(() => {
        paperFilterModal({
          ...
          callback: (grade, years, difficulty, address, examtype) => {
            ...
            // 确定筛选条件
            this.props.filterPaper(grade, years, difficulty, address, examtype);
          },
        });
      })
      .catch(function(e) {
  ...
  render() {
    ...
    const number = paperlistLength;
    const operations = (
      <FormattedMessage
        values={{ number }}
        {...{ id: 'app.text.aTotalOfnumber', defaultMessage: '共{number}套试卷' }}
      />
    );
    return (
      <div className="testFilter">
        <div className="lefttop">
          <div style={{ color: '#333', fontWeight: '600' }}>{operations}</div>
          <IconButton
