...
export default {
  ...
  state: {
    ...
    exportTranscript: null, // 导出学生成绩单数据
  },
  effects: {
    ...
    *setStudentScoreRate({ payload }, { call }) {
      const res = yield call(setStudentScoreRate, payload);
      return res;
    },
    *getStudentSubquestion({ payload, callback }, { call }) {
    ...
    *getStudentSubquestionSpeech({ payload, callback }, { call }) {
    ...
    *getStudentAnswerReport({ payload }, { call, put }) {
    ...
    // 导出成绩单
    *exportTranscript({ payload }, { all, call, put }) {
      const [res1] = yield all([call(getTranscript, payload)]);
      if (res1.responseCode === '200') {
        const resData = { transcript: res1.data };
        yield put({
          type: 'saveExportTranscript',
          payload: resData,
        });
        return res1.data;
      }
  ...
  reducers: {
    ...
    saveTaskStatus(state, action) {
      const { status, linkStatus, rectifyType } = action.payload;
      return {
        ...state,
        taskStatus: { status, linkStatus, rectifyType },
      };
    },
    ...
    saveExportTranscript(state, action) {
